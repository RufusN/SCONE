...
def makeWW(n):
    ...
    WWmap  = flux/np.max(flux)
    WW     = np.reshape(WWmap, np.size(WWmap), order='C')
    
    # write WW file
    file_name = "ww_" + str(n) + ".txt"
...
def run_sbatch(input_name):
    ...
    os.system("sbatch /home/vr339/scripts/VR/vrSlurm_SL2_3h /home/vr339/VR/cask/trrm_flux/" + input_name)

#-------------------------------------------------------------------------------
#-------------------------------      MAIN      --------------------------------
#-------------------------------------------------------------------------------

path = "/home/vr339/VR/cask/trrm_flux/"   # MAIN FOLDER PATH
os.chdir(path)

# input file name
input_name = "cask_MC_long"

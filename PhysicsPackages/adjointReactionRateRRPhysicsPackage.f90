module adjointReactionRateRRPhysicsPackage_class !currently backward
  ...
  !!   PP {
  !!     type adjointReactionRateRRPhysicsPackage;
  ...
  !!
  type, public, extends(physicsPackage) :: adjointReactionRateRRPhysicsPackage
    private
    ! Components
  ...
  end type adjointReactionRateRRPhysicsPackage
...
contains
  ...
  subroutine init(self,dict)
    class(adjointReactionRateRRPhysicsPackage), intent(inout) :: self
    ...
    character(100), parameter :: Here = 'init (adjointReactionRateRRPhysicsPackage_class.f90)'
  ...
  subroutine initPerturbation(self,dict)
    ...
    character(100), parameter :: Here = 'initPerturbation (adjointReactionRateRRPhysicsPackage_class.f90)'
  ...
  subroutine initialiseRay(self, r)
    ...
    character(100), parameter :: Here = 'initialiseRay (adjointReactionRateRRPhysicsPackage_class.f90)'
  ...
  subroutine transportSweep(self, r, ints, isActive)
    class(adjointReactionRateRRPhysicsPackage), target, intent(inout) :: self
    type(ray), intent(inout)                              :: r
  ...
  subroutine reactionRateCalc(self)
    class(adjointReactionRateRRPhysicsPackage), target, intent(inout) :: self
    integer(shortInt), save                                           :: idx, matIdx, g
    real(defFlt), save                                                :: fiss, vol
    integer(shortInt)                                                 :: cIdx
    !$omp threadprivate(fiss, vol, idx, g, matIdx)
    ...
    do cIdx = 1, self % nCells
        ...
        vol = real(self % volume(cIdx),defFlt)
        do g = 1, self % nG
            fiss = self % fission((matIdx - 1) * self % nG + g)
            idx  = self % nG * (cIdx - 1) + g

            self % ReactionRateNum(idx) = self % scalarFlux(idx) * vol * fiss
            self % ReactionRateDen(idx) = self % scalarFlux(idx) 
            self % RRNum = self % RRNum + self % scalarFlux(idx) 
            self % RRDen = self % RRDen + self % scalarFlux(idx) * vol * fiss
        end do
    end do

  end subroutine reactionRateCalc
  ...
  subroutine printSettings(self)
    class(adjointReactionRateRRPhysicsPackage), intent(in) :: self
...
end module adjointReactionRateRRPhysicsPackage_class

...
contains
  ...
  subroutine sensitivityCalculation(self, ONE_KEFF, it)
    ...
    do cIdx = 1, self % nCells
      ...
      elseif (XScase == 3) then !scatter - needs fixing
        ...
        do g = 1, self % nG 
          ...
          if (g == Sg1) then
            delta = IPVec((g - 1) * self % nG + g) * scatterXS((Sg2 - 1) * self % nG + Sg1) * XSchange

            do gIn = 1, self % nG
              if (gIn == Sg2) then
                scatter_pert = scatter_pert + scatterXS( (gIn - 1) * self % nG + g ) * IPVec((g - 1 ) * self % nG + gIn) * XSchange
              end if
            end do
          end if

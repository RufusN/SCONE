module adjointTRRMPhysicsPackage_class !currently backward
    ...
    type, public, extends(physicsPackage) :: adjointTRRMPhysicsPackage
      ...
      real(defReal), dimension(:), allocatable   :: angularFW
      real(defReal), dimension(:), allocatable   :: angularAdj
  ...
  contains
    ...
    subroutine init(self,dict)
      ...
      allocate(self % angularFW(self % nCells * self % nG))
      allocate(self % angularAdj(self % nCells * self % nG))
    ...
    subroutine cycles(self)
      ...
      self % angularAdj     = 0.0_defReal
      self % angularFW     = 0.0_defReal
    ...
    subroutine transportSweep(self, r, ints)
      ...
      real(defReal), pointer, dimension(:)                  :: angularFW, angularAdj
      ...
      do i = segCount, 1, -1
        ...
        if (i <= segCountCrit) then
          ...
          angularFW => self % angularFW((baseIdx + 1):(baseIdx + self % nG))
          angularAdj => self % angularAdj((baseIdx + 1):(baseIdx + self % nG))
          !$omp simd 
          do g = 1, self % nG
              scalarVec(g) = scalarVec(g) + delta(g) * tauBack(segIdx + g)
              angularAdj(g) = angularAdj(g)  + avgFluxVec(g)
              angularFW(g)  = angularFW(g)   + fluxRecord(segIdx + g)  
    ...
    subroutine resetFluxes(self)
      ...
      do idx = 1, size(self % scalarFlux)
        ...
        self % angularFW(idx) = 0.0_defFlt
        self % angularAdj(idx) = 0.0_defFlt
      end do
      !$omp end parallel do
    ...
    subroutine sensitivityCalculation(self, ONE_KEFF, it)
      ...
      real(defReal), dimension(:), pointer          :: FWVec, AdjVec
      ...
      do cIdx = 1, self % nCells
        do g = 1, self % nG
          ...
          if (self % volume(cIdx) > volume_tolerance) then
            self % angularFW(idx) = self % angularFW(idx) * normVol / (self % volume(cIdx))
            self % angularAdj(idx) = self % angularAdj(idx) * normVol / (self % volume(cIdx))
          else
            self % angularFW(idx) = 0.0_defFlt
            self % angularAdj(idx) = 0.0_defFlt
          end if
      ...
      do cIdx = 1, self % nCells
        ...
        FWVec => self % angularFW((baseIdx+1):(baseIdx + self % nG))
        AdjVec => self % angularAdj((baseIdx+1):(baseIdx + self % nG))
        ...
        !$omp simd
        do gIn = 1, self % nG
          fission = fission + FWVec(gIn) * nuFission(gIn) 
        end do
        fission = fission * AdjVec(gIn)
        ...
        if (XScase == 1) then
          ...
          do g = 1, self % nG 
            ...
            ! Change in XS
             ...
             delta = self % scalarflux(idx) * self % adjScalarFlux(idx) * deltaXS!AdjVec(g) * FWVec(g) * deltaXS
             print *, delta
            num(idx) = -delta
      ...
      self % sensitivity = (self % deltaKeff / ( self % keff ) ) 
        ...
        ! print *, self % angularFW, self % angularAdj
        !print *, self % deltaKeff, numsum, densum
    ...
    subroutine kill(self)
      ...
      if (allocated(self % angularFW)) deallocate(self % angularFW)
      if (allocated(self % angularAdj)) deallocate(self % angularAdj)

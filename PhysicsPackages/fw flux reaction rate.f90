module adjointBackTRRMPhysicsPackage_class
  ...
  contains
    ...
    subroutine transportSweep(self, r, ints, isActive)
      ...
      real(defFlt), pointer, dimension(:)                   :: scalarVec, sourceVec, totVec, scalarVecFW, &
                                                                sourceVecFW, deltaFW, RR, FFlux
      ...
      do i = segCount, 1, -1
        ...
        if (i <= segCountCrit) then
          ...
          !$omp simd aligned(scalarVec, scalarVecFW, RR, FFlux)
          do g = 1, self % nG
      ...
      do cIdx = 1, self % nCells
        ...
        do g = 1, self % nG 
          ...
          ! if (self % volume(cIdx) > volume_tolerance) then
            ...
            self % FWFlux(idx) = self % FWFlux(idx) * (self % volume(cIdx))                                  
          ! else
    ...
    subroutine adjointSourceUpdateKernel(self, cIdx)
      ...
      do g = 1, self % nG
        ...
        ! print *, self % detectorInt(1)
        ! print *, detmat
        ! if (detMat) then
        if (material == self % detectorInt(1)) then
      ...
      if (allocated(self % intMatIdx)) then
        ...
        do i = 1, size(self % detectorInt)
          ...
          do cIdx = 1, self % nCells
            ...
            if (self % detectorInt(i) == matIdx) then
              vol = self % volume(cIdx) 
              ...
              do g = 1, self % nG
                idx = (cIdx - 1)* self % nG + g
                Sigma = real(mat % getFissionXS(g, self % rand),defFlt)
                res = res + self % fluxScores(idx,1) * vol * Sigma !* self % fixedSource(idx)

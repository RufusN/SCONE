module adjointTRRMPhysicsPackage_class !currently backward
  ...
  type, public, extends(physicsPackage) :: adjointTRRMPhysicsPackage
    ...
    integer(shortInt)  :: NSegMax     = 20000
    integer(shortInt)  :: NSegMaxTemp = 20000
...
contains
  ...
  subroutine transportSweep(self, r, ints, isActive)
    ...
    real(defFlt), pointer, dimension(:)                   :: scalarVec, sourceVec, totVec, scalarVecFW, sourceVecFW, &
                                                             deltaFW, avgRecordFW
    real(defReal), pointer, dimension(:)                  :: angularProd
    real(defReal), dimension(3)                           :: r0, mu0
    real(defReal), dimension(self % NSegMax*2)            :: lenBack
        ...
        if (tally) then
          if (oversized) then
            ...
            do g = 1, self % nG
              ...
              avgRecord((segCount - 1) * self % nG + g) = (sourceVec(g) + delta(g))!fluxVec(g) + delta(g)*tau(g) / 2
            end do
          end if
        end if
    ...
    do i = segCount, 1, -1
      ...
      if (oversized) then
        ...
        cIdx = cIdxBack(i)
        avgRecordFW => avgRecord(((i - 1) * self % nG + 1):(i * self % nG))
      end if
      ...
      if (i <= segCountCrit) then
        ...
        !$omp simd
        do g = 1, self % nG
          avgFluxVec(g) = (sourceVec(g) + delta(g)) !fluxVec(g) + delta(g)*tau(g) / 2 !
        end do

        call OMP_set_lock(self % locks(cIdx))

        scalarVec => self % adjScalarFlux((baseIdx + 1):(baseIdx + self % nG))
        angularProd => self % angularIP((baseIdx * self % nG + 1):(baseIdx * self % nG + self % nG * self % nG ))
        ...
        if (isActive) then
          do g = 1, self % nG
            associate(angProd => angularProd((self % nG * (g - 1) + 1):(self % nG * self % nG)))
              !$omp simd 
              do gIn = 1, self % nG
                angProd(gIn) = angProd(gIn) + avgFluxVec(g) * avgRecordFW(gIn)!(sourceVecFW(gIn) + deltaFW(gIn))
              end do
            end associate
          end do
        end if
  ...
  subroutine sensitivityCalculation(self, cIdx, ONE_KEFF, it, numSum, denSum)
    ...
    real(defFlt), dimension(:), pointer           :: nuFission, chi, capture, scatterXS, & !total,
                                                      fissVec, scatterVec
    ...
    IPVec => self % angularIP((baseIdx * self % nG + 1):(baseIdx * self % nG + self % nG * self % nG))
    !print *, cIdx
    ! if (size(IPVec)<=48) then
    ! print *, size(IPVec)
    ! end if
    !print *, baseIdx * self % nG + 1
    !print *, baseIdx * self % nG + self % nG * self % nG
    !print *, '-------------'
    !total => self % sigmaT((matIdx + 1):(matIdx + self % nG))
    nuFission => self % nuSigmaF((matIdx + 1):(matIdx + self % nG))
    ...
    do g = 1, self % nG
      ...
      ! if (self % volume(cIdx) > volume_tolerance) then
        denSum = denSum + fission * norm * self % volume(cIdx)
      ! end if
    end do
    ...
    if (mat == self % matPert .and. self % XStype == 1) then ! capture - complete 
      !do i = 1, size(self % energyId)
        ...
        !if (self % volume(cIdx) > volume_tolerance) then
          numSum = numSum - delta * norm * self % volume(cIdx)
        !end if
      !end do
    ...
    elseif (mat == self % matPert .and. self % XStype == 3) then !scatter - complete
      ...
      g1Pert = 1!self % energyId(i)
      g2Pert = 2!self % energyId(i+1)
      scatterVec => scatterXS((self % nG * (g2Pert - 1) + 1):(self % nG * self % nG))
      !print *, (self % nG * (g2Pert - 1) + 1)
      !print *, (self % nG * g2pert)
      !print *, size(scatterVec)
      !print *, scatterVec(g1Pert) * self % XSchange, scatterVec(g1Pert) * self % XSchange + scatterVec(g1Pert)
      ! print *, scatterVec(g1pert)
      delta = 0.0_defFlt
      scatter_pert = 0.0_defFlt
      delta = IPVec((g1pert - 1) * self % nG + g1Pert) * scatterVec(g1Pert) * &
                    self % XSchange   
          
      scatter_pert = scatterVec(g1Pert) * self % XSchange * &
                                  (IPVec((g2Pert - 1) * self % nG + g1Pert))
      ! print *, (g2pert - 1) * self % nG + g2Pert, (g1Pert - 1) * self % nG + g2Pert

      ! associate(angProd => IPVec((self % nG * (g1Pert - 1) + 1 ):(self % nG * g1Pert)))
      ! delta = angProd(g1Pert) * scatterVec(g1Pert) * &
      !       self % XSchange   
          
      ! scatter_pert = scatterVec(g1Pert) * self % XSchange * &
      !                             angProd(g1Pert)
      ! end associate
      ! delta = self % angularIP(baseIdx * self % nG + (g1Pert - 1) * self % nG + g1Pert) * scatterVec(g1Pert) * &
      !                   self % XSchange   
          
      ! scatter_pert = scatterVec(g1Pert) * self % XSchange * &
      !                       self % angularIP(baseIdx * self % nG + (g2Pert - 1) * self % nG + g1Pert)

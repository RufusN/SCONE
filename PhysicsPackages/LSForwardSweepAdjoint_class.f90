...
contains
  ...
  subroutine transportSweep(self, r, ints)
      ...
      type(ray), intent(inout)                              :: r
      integer(longInt), intent(out)                         :: ints
      integer(shortInt)                                     :: matIdx, g, cIdx, idx, event, matIdx0, baseIdx, centidx, momidx
      real(defReal)                                         :: totalLength, length
      ...
      real(defReal)                                         :: len2_12
      real(defFlt), dimension(self % nG)                    :: tau, &
                                                                F1, F2, Gn, G1, G2, H, flatQ, gradQ, &
                                                                xInc, yInc, zInc, fluxVec0
      real(defFlt), pointer, dimension(:)                   :: xGradVec, yGradVec, zGradVec, &
                                                              xMomVec, yMomVec, zMomVec
      real(defFlt)                                          ::  lenFlt2_2
      real(defReal), dimension(3)                           :: rC, r0Norm, rNorm                                                  
      ...
      do while (totalLength < self % termination)
        ...
        if (cIdx > 0 .and. length > self % skipLength) then
          ...
          do g = 1, self % nG
            ...
            flatQ(g) = flatQ(g) + sourceAdjoint(g)
        ...
        !$omp simd
          ...
          !$omp simd
          do g = 1, self % nG
            deltaAdjoint(g) = (fluxAdjoint(g) - flatQ(g)) * F1(g)  & 
          ...
          !$omp simd
          do g = 1, self % nG
            fluxVec0(g) = fluxAdjoint(g)
          end do
    
          ! Flux vector update
          !$omp simd
          do g = 1, self % nG
            fluxAdjoint(g) = fluxAdjoint(g) - deltaAdjoint(g) * tau(g) 
          end do
          ...
          if (activeRay) then
              ...
              do g = 1, self % nG
                  ...
                  flatQ(g) = flatQ(g) * lenFlt + deltaAdjoint(g) * lenFlt
              end do
              ...
              !$omp simd aligned(scalarVec, scalarAdjoint, xMomVec, yMomVec, zMomVec)
              do g = 1, self % nG
                scalarVec(g) = scalarVec(g) + delta(g) 
                scalarAdjoint(g) = scalarAdjoint(g) + deltaAdjoint(g) * length
  ...
  subroutine adjointNormaliseFluxAndVolume(self, lengthPerIt, it)
    ...
    !$omp threadprivate(total, vol, idx, g, matIdx, dIdx, mIdx, invvol)
    ...
    do cIdx = 1, self % nCells
      ...
      do g = 1, self % nG
        ...
        if (vol > volume_tolerance) then
          self % adjScalarFlux(idx) = self % adjScalarFlux(idx) * norm / ( vol)
          self % adjScalarX(idx) = self % adjScalarX(idx) * norm / ( vol)
          self % adjScalarY(idx) = self % adjScalarY(idx) * norm / ( vol)
          self % adjScalarZ(idx) = self % adjScalarZ(idx) * norm / ( vol)
        end if
